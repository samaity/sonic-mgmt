...
class IpRouteModule(object):
    ...
    def get_interfaces(self, prefix):
        """
        Get the list of interfaces for a particular prefix using 'show ip route <prefix>' command
        """

        try:
            rc, self.out, err = self.module.run_command('docker exec -i bgp vtysh -c "show ip route ' + prefix + '"', executable='/bin/bash', use_unsafe_shell=True)

        except Exception as e:
            err_msg = "Exception occured while trying to get the list of interfaces! " + str(e)
            self.module.fail_json(msg=err_msg)

        else:
            if rc != 0:
                self.module.fail_json(msg="Command 'show ip route <prefix>' failed with non-zero return code! out=%s, err=%s" %(self.out, self.err))

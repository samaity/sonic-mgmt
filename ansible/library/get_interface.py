...
short_description: Retrieve the interface through which traffic is flowing to the destination
description:
...
class GetInterfaceModule(object):
    ...
    def get_iface(self):
        """
        Get the interface from 'show interface counter' output through which traffic is flowing
        """
        ...
        try:
            ...
            rc, self.out, err = self.module.run_command(cmd, executable='/bin/bash', use_unsafe_shell=True)
            ...
            if rc != 0:
                self.module.fail_json(msg="Command 'show interface counter' failed with non-zero return code! out=%s, err=%s" %(self.out, self.err))

        lines = self.out.splitlines()

        # Ignore 1st 4 lines, and then get only the first 16 interfaces: Ethernet0 to Ethernet 16
        for line in lines[4:19]:
            iface_rx[line.split()[0]] = line.split()[2]
